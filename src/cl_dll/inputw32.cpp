...
===========
*/
void IN_SyncMouseMode (const bool & bSyncRI = false, const int & iActive = mouseactive)
{
#ifdef _WIN32
	const bool bRIIsOn = bSyncRI ? (CVAR_GET_FLOAT("m_rawinput") != 0.0f) : m_bRawInput;
#else
	const bool bRIIsOn = true;
#endif
	if (iActive && !(iMouseInUse || gHUD.m_iIntermission || g_iVisibleMouse) && bRIIsOn)
	{
#ifdef _WIN32
		if (bSyncRI) m_bRawInput = bRIIsOn;	// It must be updated BEFORE turning RMM on
#endif
...
#ifdef _WIN32
		if (bSyncRI) m_bRawInput = bRIIsOn;	// It must be updated AFTER turning RMM off
#endif
	}
}
...
#endif
		IN_SyncMouseMode(true, 1);	// Vit_amiN: sync with the m_bRawInput update
		mouseactive = 1;
	}
}
...
void CL_DLLEXPORT IN_DeactivateMouse (void)
{
	if (mouseinitialized)
	{
		...
		IN_SyncMouseMode(true, 0);	// Vit_amiN: sync with the m_bRawInput update
		mouseactive = 0;
	}
}
...
#ifdef _WIN32
	// Vit_amiN: the timer check must be first because it changes m_bRawInput
	if ( gpGlobals && gpGlobals->time - s_flRawInputUpdateTime > 1.0f )
	{
		s_flRawInputUpdateTime = gpGlobals->time;
		IN_SyncMouseMode(true);	// Vit_amiN: sync with the m_bRawInput update
	}
...
{
	IN_SyncMouseMode();	// Vit_amiN: simple sync
