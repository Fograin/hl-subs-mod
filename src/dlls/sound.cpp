						   ...
						   int flags, int pitch)
{
	...
	UTIL_ShowMessagePVS( sample, entity->v.origin ); // Fograin92: Trigger ShowMessage on every sound
}
...
void EMIT_SOUND_SUIT(edict_t *entity, const char *sample)
{
	...
	// Fogrian92: Block NPC random talk, while HEV logon sound is played
	if(sample == "!HEV_A0")
		CTalkMonster::g_talkWaitTime = gpGlobals->time + 12.0;
	else if(sample == "!HEV_AAx")
		CTalkMonster::g_talkWaitTime = gpGlobals->time + 38.0;
}
