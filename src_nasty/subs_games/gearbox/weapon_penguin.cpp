...
void CPenguinGrenade :: Killed( entvars_t *pevAttacker, int iGib )
{
	...
	// Penguins do a bit of radius damage when they pop, and that radius damage will
	// continue to call this function unless we acknowledge the Penguin's death now. (sjb)
	...
	if ( pOther->pev->takedamage && m_flNextAttack < gpGlobals->time )
	{
		...
		if (tr.pHit == pOther->edict())
		{
			...
			if (tr.pHit->v.modelindex != pev->modelindex)
			{
				...
				ClearMultiDamage( );
				pOther->TraceAttack(pev, 15, gpGlobals->v_forward, &tr, DMG_SLASH ); 
				if (m_hOwner != NULL)
					ApplyMultiDamage( pev, m_hOwner->pev );
				else
					ApplyMultiDamage( pev, pev );

				pev->dmg += 50; // add more explosion damage
...
#ifndef CLIENT_DLL
			CBaseEntity *pPenguin = CBaseEntity::Create( "monster_penguin", tr.vecEndPos, m_pPlayer->pev->v_angle, m_pPlayer->edict() );
			pPenguin->pev->velocity = gpGlobals->v_forward * 200 + m_pPlayer->pev->velocity;
#endif
