...
cvar_t *scroll_speed;
cvar_t *fade_speed;
const char *pMessageTempik; // Fograin92: Var used to store name of the last added sentence
...
void SubtitleMessageAdd( client_textmessage_t *tempMessage )
{
	...
	{
		// Fograin92: Check if new sentence name is different than sentence before
		// Little tricky but, it will prevent multiple subtitles while ingame fake echo is present (2x same sentence played at almost the same time)
		if(pMessageTempik != tempMessage->pName)
		{
			pMessageTempik = tempMessage->pName;
...
CSubtitle::CSubtitle() : Panel(XRES(10), YRES(355), XRES(598), YRES(95))
{	
	m_pLayer = new Panel;
...
{	
	...
	if (m_pCur)
	{
		if (m_fCurStartTime + m_pCur->m_fHoldTime <= curtime)
		{
			...
			if (m_pLayer->getChildCount() == 0)
			{
				m_pWait = NULL;
				setVisible(false);
				//gEngfuncs.Con_Printf("Subtitles: All subtitles disapeared\n");
				pMessageTempik = ""; // Fograin92: Since all subtitles disapeared, we can clear this var
				return;
			}
	...
	lasttime = curtime;
	//Panel::paintBackground(); // Paint BG for whole panel
}