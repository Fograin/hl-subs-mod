...
#include "vgui_MSG_Chapter.h"
#include "VGUI_TextImage.h"

cvar_t *ChapterMSG_enabled;

Font* FontFromMessageZ(const char* &ptext)
{
...
{
	if (!ChapterMSG_enabled->value)
		return;

	if (gViewPort && gViewPort->m_pChapterMSG)
	{
		gViewPort->m_pChapterMSG->AddMessage( tempMessage );
	}
	else
		gEngfuncs.Con_Printf("Subtitle error: CChapterMSG or ViewPort is not constructed!\n");
}

void CChapterMSGInit()
{
	ChapterMSG_enabled = gEngfuncs.pfnRegisterVariable( "MSG_Chapter", "1", 0 );
}

void CChapterMSGTextPanel::paintBackground()
{
	float fade = 0.2;
	...
	if (gViewPort && gViewPort->m_pChapterMSG) // hm..
	{
		if (gViewPort->m_pChapterMSG->m_pCur == this)
		{
			float dietime = gViewPort->m_pChapterMSG->m_fCurStartTime + m_fHoldTime;
...
void CChapterMSGTextPanel::paint()
{
	int mr, mg, mb, ma;
	int ix, iy;
...
CChapterMSG::CChapterMSG() : Panel(XRES(5), YRES(5), XRES(630), YRES(470))
{	
	m_pLayer = new Panel;
...
void CChapterMSG::AddMessage( client_textmessage_t *msg )
{
	...
	Font *pFont = FontFromMessageZ(pText);

	// Fograin92: Give us enough place to render shit
	m_pLayer->setSize(ScreenWidth-msg->x, ScreenHeight-msg->y);

	int new_X, new_Y;
	CChapterMSGTextPanel *text = new CChapterMSGTextPanel(pText, msg->x, msg->y, ScreenWidth-msg->x, ScreenHeight-msg->y );
...
void CChapterMSG::paintBackground()
{	
