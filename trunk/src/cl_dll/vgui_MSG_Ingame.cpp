...
// $effect 6 = ID for misc/random ingame messages
...
#include "vgui_MSG_Ingame.h"
#include "VGUI_TextImage.h"

cvar_t *IngameMSG_enabled;

Font* FontFromMessageX(const char* &ptext)
{
...
{
	if (!IngameMSG_enabled->value)
		return;

	if (gViewPort && gViewPort->m_pIngameMSG)
	{
		gViewPort->m_pIngameMSG->AddMessage( tempMessage );
	}
	else
		gEngfuncs.Con_Printf("Subtitle error: CIngameMSG or ViewPort is not constructed!\n");
}

void IngameMSGInit()
{
	IngameMSG_enabled = gEngfuncs.pfnRegisterVariable( "MSG_Ingame", "1", 0 );
}

void CIngameMSGTextPanel::paintBackground()
{
	float fade = 0.2;
	...
	if (gViewPort && gViewPort->m_pIngameMSG) // hm..
	{
		if (gViewPort->m_pIngameMSG->m_pCur == this)
		{
			float dietime = gViewPort->m_pIngameMSG->m_fCurStartTime + m_fHoldTime;
...
void CIngameMSGTextPanel::paint()
{
	int mr, mg, mb, ma;
	int ix, iy;
...
CIngameMSG::CIngameMSG() : Panel(XRES(5), YRES(5), XRES(630), YRES(470))
{	
	m_pLayer = new Panel;
...
void CIngameMSG::AddMessage( client_textmessage_t *msg )
{
	...
	Font *pFont = FontFromMessageX(pText);

	// Fograin92: Give us enough place to render shit
	m_pLayer->setSize(ScreenWidth-msg->x, ScreenHeight-msg->y);

	int new_X, new_Y;
	CIngameMSGTextPanel *text = new CIngameMSGTextPanel(pText, msg->x, msg->y, ScreenWidth-msg->x, ScreenHeight-msg->y );
...
void CIngameMSG::paintBackground()
{	
