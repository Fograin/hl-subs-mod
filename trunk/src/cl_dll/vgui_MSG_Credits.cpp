...
// $effect 5 // ID for chapter title messages
...
#include "vgui_MSG_Credits.h"
#include "VGUI_TextImage.h"

cvar_t *CreditsMSG_enabled;

Font* FontFromMessageZz(const char* &ptext)
{
...
{
	if (!CreditsMSG_enabled->value)
		return;

	if (gViewPort && gViewPort->m_pCreditsMSG)
	{
		gViewPort->m_pCreditsMSG->AddMessage( tempMessage );
	}
	else
		gEngfuncs.Con_Printf("Subtitle error: CCreditsMSG or ViewPort is not constructed!\n");
}

void CCreditsMSGInit()
{
	CreditsMSG_enabled = gEngfuncs.pfnRegisterVariable( "MSG_Credits", "1", 0 );
}

void CCreditsMSGTextPanel::paintBackground()
{
	float fade = 0.2;
	...
	if (gViewPort && gViewPort->m_pCreditsMSG) // hm..
	{
		if (gViewPort->m_pCreditsMSG->m_pCur == this)
		{
			float dietime = gViewPort->m_pCreditsMSG->m_fCurStartTime + m_fHoldTime;
...
void CCreditsMSGTextPanel::paint()
{
	int mr, mg, mb, ma;
	int ix, iy;
...
CCreditsMSG::CCreditsMSG() : Panel(XRES(5), YRES(5), XRES(630), YRES(470))
{	
	m_pLayer = new Panel;
...
void CCreditsMSG::AddMessage( client_textmessage_t *msg )
{
	...
	Font *pFont = FontFromMessageZz(pText);

	// Fograin92: Give us enough place to render shit
	m_pLayer->setSize(ScreenWidth-msg->x, ScreenHeight-msg->y);

	int new_X, new_Y;
	CCreditsMSGTextPanel *text = new CCreditsMSGTextPanel(pText, msg->x, msg->y, ScreenWidth-msg->x, ScreenHeight-msg->y );
...
void CCreditsMSG::paintBackground()
{	
