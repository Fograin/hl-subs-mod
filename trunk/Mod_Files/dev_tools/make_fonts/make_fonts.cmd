...
for /F "eol=; delims=	 tokens=1-4,* usebackq" %%a in ("%Fonts_Lst%") do (
	set "File_Name=%Fonts_Dir%\%%~a.tga"
	set "Font_Name=%%~b"
	set "Font_Style=%%~c"
	set "Font_Size=%%~d"
	call :MakeTGAFont "File_Name" "Font_Name" "Font_Style" "Font_Size"
)>>"%Fonts_Log%"
...
set "Cmd_Line=-font "%Font_Name%" -pointsize %Font_Size%"
